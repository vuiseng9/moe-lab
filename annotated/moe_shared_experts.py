...
class MoE(nn.Module):
    def __init__(self, D, E, K, DFF, ES, bias=False):
        ...
        self.ES = ES     # number of shared experts
        ...
        # shared experts
        self.shared = nn.ModuleList([GluMLP(D, DFF, bias=bias) for _ in range(ES)])

    def extra_repr(self):
        return f"[K:E] + ES = {self.K}:{self.E} + {self.ES},\n" \
                "\tK activated out of E experts per token, \n" \
                "\tES shared experts."
    ...
    def forward(self, x): 
        ...
        # accumulating shared experts 
        for seid in range(ES):
            moe_outputs += self.shared[seid](_x)
...
if __name__ == "__main__":
    ...
    ES = 4  # number of shared experts
    # expert 
    DFF = D*4 # expert dimension

    moe_layer = MoE(D, E, K, DFF, ES)
    print(moe_layer)
