...
class MoeBlk(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        assert expert_capacity is None, "Temporarily disable expert capacity to focus on grouped gemm optimization. Will re-enable in future."
...
class MoedlForCausalLM(MoedlPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        lb_loss = None
        if self.training and self.config.num_experts > 1 and self.config.lb_coeff > 0.0 and loss is not None:
