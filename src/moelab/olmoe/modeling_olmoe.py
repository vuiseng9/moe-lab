...
from transformers.models.olmoe.modeling_olmoe import OlmoeForCausalLM, OlmoeMLP, load_balancing_loss_func
...
class MoelabOlmoeForCausalLM(OlmoeForCausalLM):
    ...
    def monkey_patch(self):
        # append shared experts to OlmoeSparseMoeBlock
        for blk in self.model.layers:
            moe = blk.mlp
            moe.num_shared_experts = self.config.num_shared_experts
            
            if self.config.num_shared_experts > 0:
                moe.shared = torch.nn.ModuleList([OlmoeMLP(self.config) for _ in range(self.config.num_shared_experts)])

        # monkey patch OlmoeSparseMoeBlock forward to shared_expert_forward
        MoeClass = type(moe)
        MoeClass.forward = shared_expert_forward
...
def shared_expert_forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
    ...
    for s in range(self.num_shared_experts):
        final_hidden_states += self.shared[s](hidden_states)
